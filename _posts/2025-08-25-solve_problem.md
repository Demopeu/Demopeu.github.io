...
- __핵심 식__: 전체 격자 수 `T = brown + yellow`.
  - 테두리만 갈색이므로 내부 노란 격자는 `(w - 2) * (h - 2) = yellow`.
  - 따라서 `w >= h >= 3` 이고, `w * h = T` 를 만족하는 약수 쌍 중 위 조건을 만족하는 것을 찾으면 됩니다.
- __전략__: `h`를 3부터 `sqrt(T)`까지 순회하며 `T % h === 0` 인 경우 `w = T / h` 를 구하고 `(w - 2) * (h - 2) === yellow` 인지 검사합니다.
- __복잡도__: 약수 탐색이므로 시간복잡도는 O(√T) 입니다.
